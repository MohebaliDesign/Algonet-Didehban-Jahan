...
| Native text/search inputs                                    | `Input`, `CommandInput`, `InputGroup`                 | `AppShell`, `WorldMap`, `ProductPages`                               | Global Search, layer search, country/report/source filters     | Complete                                                                            |
...
| Persistent map-layer panel                                   | `Popover` + `Button` + `Badge`                        | `MapOverlayControls`, `WorldMap`                                     | Compact layer trigger and viewport-safe floating panel         | Complete                                                                            |
| Layer toggles                                                | `Checkbox` + `Label` + `ScrollArea`                   | `MapOverlayControls`, `WorldMap`                                     | Searchable map-layer list                                      | Complete                                                                            |
...
| Application navigation controls                              | `Sidebar`, `SidebarMenu`, `SidebarTrigger`, `Tooltip` | `AppShell`                                                           | 240px RTL/LTR rail with stable icon-only and mobile states     | Complete; React Router links remain product-owned                                   |
| User identity and prototype role preview                     | `SidebarFooter`, `Avatar`, `DropdownMenu`             | `AppShell`, `WorkspaceProvider`                                      | Canonical footer profile menu and local-only typed role state  | Complete; deliberately not persisted and never production authorization             |
| Authorized source/data management entry                      | `DropdownMenuItem`, responsive `Sheet`, `ScrollArea`  | `AppShell`, `ProductPages`                                           | Existing Data Management content embedded for authorized roles | Complete; legacy `/data` route is hidden from primary navigation and remains gated  |
| Product panels                                               | `Card` + `Separator` + `Collapsible`                  | `ModuleFrame`, Foundation                                            | `ModuleFrame`, analytical content groups                       | Complete                                                                            |
...
| Geographic rendering surface                                | Highcharts Maps                                       | `WorldMap`, `SecurityWorldMap`                                       | Accurate countries, choropleth, clustered points, corridors   | Documented exception: Highcharts renders geography; all surrounding controls use shadcn |

## Installed UI source

The repository owns the generated shadcn source under `src/components/ui/`: `Alert`, `Avatar`, `Badge`, `Button`, `Card`, `Chart`, `Checkbox`, `Collapsible`, `Command`, `Dialog`, `Direction`, `DropdownMenu`, `Empty`, `Input`, `InputGroup`, `Label`, `Popover`, `Progress`, `ScrollArea`, `Select`, `Separator`, `Sheet`, `Sidebar`, `Skeleton`, `Slider`, `Sonner`, `Table`, `Tabs`, `Toggle`, `ToggleGroup`, and `Tooltip`.
...
- Sheet sides, popover/dropdown alignment, layout mirroring, and technical-string direction follow the current locale without duplicated RTL/LTR trees.
...
The repository check searches for direct `<button>`, `<input>`, `<select>`, `<textarea>`, and `<table>` in `src` excluding `src/components/ui`. Native elements inside official shadcn implementations are expected. Highcharts Maps is the approved geographic-rendering exception: it owns country geometry, map/mapline/mappoint series, clustering, labels, map navigation, and geographic tooltips. shadcn continues to own the map/list switch, layer controls, state surfaces, timeline, and contextual Sheet. Didehban semantic CSS tokens are resolved into Highcharts options; they remain the visual authority.
